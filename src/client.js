...
const parseApiKey = raw => {
    if (!raw || !raw.trim()) {
        throw new Error(
            'GREENLINE_API_KEY is required. Generate one in GreenLine → Org Settings → Integrations → API Key.'
        );
    }
    const idx = raw.indexOf(':');
    if (idx <= 0 || idx === raw.length - 1) {
        throw new Error(
            'GREENLINE_API_KEY is malformed — expected "accessId:apiKey". Re-copy it from Org Settings → Integrations → API Key.'
        );
    }
...
const initApiClient = () => {
    const { accessId, secret } = parseApiKey(process.env.GREENLINE_API_KEY);
    clientConfig = {
        accessId,
        secret,
        baseUrl: (process.env.GREENLINE_BASE_URL && process.env.GREENLINE_BASE_URL.trim()) || DEFAULT_BASE_URL,
    };
};
...
const handleApiError = error => {
    if (axios.isAxiosError(error)) {
        ...
        if (status === 401) {
            return 'Authentication failed. Check GREENLINE_API_KEY — get a fresh one from Org Settings → Integrations → API Key.';
        }
        if (status === 403) {
        ...
        if (status && status >= 500) {
            return `Upstream API error (${status}). Try again shortly; if it persists this is a GreenLine issue.`;
        }
        if (error.code === 'ECONNABORTED') {
            return 'Request timed out. Try again or narrow the request (filters, smaller `count`).';
        }
        if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
            return 'Could not reach the GreenLine API. Check GREENLINE_BASE_URL or your network.';
        }
    }

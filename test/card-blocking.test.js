...
test('update_card accepts a boolean blocked flag', () => {
    const field = tools.greenline_update_card.inputSchema.blocked;
    assert.ok(field, 'greenline_update_card has no blocked field');
...
test('update_card accepts a blockedReason string', () => {
    const field = tools.greenline_update_card.inputSchema.blockedReason;
    assert.ok(field, 'greenline_update_card has no blockedReason field');
...
test('update_card documents blocking in its prose description', () => {
    assert.match(tools.greenline_update_card.description, /blocked/);
});

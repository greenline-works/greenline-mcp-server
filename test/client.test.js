...
test('stdio path: makeApiRequest uses the env credential when there is no call context', async t => {
    process.env.GREENLINE_API_KEY = 'envAccess:envSecret';
    delete process.env.GREENLINE_BASE_URL;
    initApiClient();
    ...
    assert.match(calls[0].url, /integrations\.greenline\.works\/v1\/thing$/);
});
...
test('runWithCredentials with a bearerToken forwards a Bearer header, not Basic auth', async t => {
    ...
    await runWithCredentials(
        { bearerToken: 'opaque-oauth-token', baseUrl: 'https://integrations.greenline.works/v1' },
...
test('runWithCredentials binds call-scoped credentials, then falls back after', async t => {
    ...
    await runWithCredentials(
        { accessId: 'reqAcc', secret: 'reqSek', baseUrl: 'https://integrations.greenline.works/v1' },
